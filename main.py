...
class PredictionScreen(Screen):
    ...
    def predict(self, instance):
        try:
            ...
            for num in range(n_columns):
                y = data.iloc[:, num]  # Extraire la colonne à prédire
                X = data.drop(columns=data.columns[num])  # Utiliser toutes les autres colonnes comme caractéristiques
            ...
            last_row = data.iloc[-1].drop(data.columns[-2]).to_frame().T

            self.predictions = []
            for num in range(n_columns):
                model = models[f'num_{num}']
                X_new = np.array(last_row).reshape(1, -1)
                predicted_number = model.predict(X_new)[0]
                self.predictions.append(int(predicted_number))
    ...
    def draw_numbers(self):
        ...
        for number in all_numbers:
            color = [1, 1, 1, 1]  # Initially white for all numbers
            if number in self.predictions:
                color = [0, 1, 0, 1]  # Change to red for predicted numbers
